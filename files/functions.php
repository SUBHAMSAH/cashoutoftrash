        ...
        $errors[] = "Password must include at least one letter!";
	        ...
	        if(!empty($errors)){
				...
				if(register_user($firstname,$lastname,$username,$email,$password,$image)){
					...
					set_message("<p class='bg-success text-center'>Please check your email for activation link and login again.</p>");
...
function register_user($firstname,$lastname,$username,$email,$password){
	...
	if(email_exists($email)){
	  ...
	  http://cashoutoftrash.org/activate.php?email=$email&code=$validation_code
		
		
		";
	   $headers="From:noreply@cashoutoftrash.org";
	...
	if($_SERVER['REQUEST_METHOD']=="GET"){
		...
		if(isset($_GET['email'])){
			...
			if(row_count($result)==1){
				...
				$sql2="UPDATE usertry SET active=1,validation_code=0 WHERE user_email='".escape($email)."' AND validation_code='".escape($validation_code)."'";
				$result2=query($sql2);
			confirm($result2);
...
function validate_user_login()
	...
	function login_user($email,$password)
	{
		...
		$sql3="SELECT * FROM usertry WHERE user_email='".escape($email)."' AND active=1";
		$result3=query($sql3);
...
function recover_password(){
	...
	if($_SERVER['REQUEST_METHOD']=="POST"){
		...
		if(email_exists($email)){
			...
			Click here to reset your password : http://cashoutoftrash.org/code.php?email=$email&code=$validation_code
